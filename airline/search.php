...
if (!empty($from) && !empty($to) && !empty($date)) {
    ...
    $sql = "SELECT flight_id, flight_number, scheduled_dep_time, fl_status 
            FROM flight 
            WHERE source = ? 
            AND destination = ? 
            AND scheduled_date = ? 
            AND (scheduled_date > CURDATE() OR (scheduled_date = CURDATE() AND scheduled_dep_time > CURTIME())) 
            AND fl_status = 'Scheduled'
            ORDER BY scheduled_dep_time ASC";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "Flight: " . $row['flight_number'] . " - Departs: " . $row['scheduled_dep_time'];
