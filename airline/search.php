...
<body>
    ...
    if (!empty($from) && !empty($to) && !empty($date)) {
        ...
        $sql = "SELECT f.flight_id, f.flight_number, f.scheduled_dep_time, f.fl_status, 
                ...
                AND (f.scheduled_date > CURDATE() OR (f.scheduled_date = CURDATE() AND f.scheduled_dep_time > CURTIME())) 
                AND f.fl_status = 'Scheduled'    
                ORDER BY f.scheduled_dep_time ASC";
        ...
        if ($result->num_rows > 0) {
            ...
            <table>
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): 
                        $formattedTime = date("h:i A", strtotime($row['scheduled_dep_time']));
                    ?>
                    <tr>
            ...
            echo "<blockquote>No flights found for this route and date. <a href='index.php'>Go back</a></blockquote>";
        }
    }
    ?>
